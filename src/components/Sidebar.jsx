...
import {         
  ...
  FaCoins, FaCalculator, FaEllipsisH, FaUser, FaGamepad         
} from 'react-icons/fa';         
...
const Sidebar = ({ isOpen, toggleSidebar }) => {        
  ...
  // 🚀 Token Creator Redirect (Port 5173) 
  ...
  // 🛠️ Solthubfun Redirect (Port 5176) 
  ...
  return (         
    <aside className={`fixed top-0 left-0 h-full bg-[#0b1426] border-r border-cyan-500/10 z-[60] transition-all duration-300 ease-in-out ${isOpen ? 'w-64' : 'w-20'}`}>         
      ...
      <div className="flex items-center justify-between p-5 h-16 border-b border-cyan-500/5">         
        {isOpen && (         
          <h2 className="text-yellow-400 font-bold text-xl tracking-tighter uppercase italic">SoltHub</h2>         
        )}         
        <button onClick={toggleSidebar} className={`bg-yellow-500 hover:bg-yellow-400 text-black p-2 rounded-md transition-all mx-auto ${isOpen ? '' : 'w-10'}`}>        
          {isOpen ? '✕' : '☰'}         
        </button>         
      </div>         
      ...
      <nav className="flex flex-col gap-1 p-3 overflow-y-auto h-[calc(100%-120px)] scrollbar-hide">         
        ...
        {/* Main Hub Internal Route */} 
        <MenuItem to="/" icon={<FaHome />} label="Home" isOpen={isOpen} />         
          
        {/* Independent Project Redirections */} 
        ...
        >         
          <span className="text-xl"><FaShieldAlt /></span>         
...
// For Internal React-Router Routes 
