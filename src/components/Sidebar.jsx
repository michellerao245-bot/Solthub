...
const Sidebar = ({ isOpen, toggleSidebar }) => {  
  ...
  // 🚀 Redirect Functions (Brackets fixed)
  ...
  return (  
    <aside className={`fixed top-0 left-0 h-full bg-[#0b1426] border-r border-cyan-500/10 z-[60] transition-all duration-300 ease-in-out ${isOpen ? 'w-64' : 'w-20'}`}>  
      ...
      <nav className="flex flex-col gap-1 p-3 overflow-y-auto h-[calc(100%-120px)] scrollbar-hide">  
        ...
        <a href="https://smarttools.vercel.app" target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 px-4 py-3 text-slate-400 hover:bg-cyan-500/10 hover:text-cyan-400 rounded-lg transition-all">  
          <span className="text-xl"><FaShieldAlt /></span> {isOpen && <span className="text-sm font-medium">Smart Tools</span>}  
        </a>  
